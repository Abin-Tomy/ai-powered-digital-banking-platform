...
function getPasswordStrength(pw: string): { label: string; color: string; width: string; level: number } {
  if (pw.length === 0) return { label: "", color: "", width: "0%", level: 0 };
  const hasNumber = /\d/.test(pw);
  const hasSpecial = /[^A-Za-z0-9]/.test(pw);
  if (pw.length < 8) return { label: "Weak", color: "bg-red-500", width: "25%", level: 1 };
  if (pw.length >= 8 && !hasNumber) return { label: "Fair", color: "bg-orange-500", width: "50%", level: 2 };
  if (pw.length >= 8 && hasNumber && !hasSpecial) return { label: "Good", color: "bg-yellow-500", width: "75%", level: 3 };
  return { label: "Strong", color: "bg-emerald-500", width: "100%", level: 4 };
}
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = data?.email?.[0] || data?.detail || "Registration failed. Please try again.";
